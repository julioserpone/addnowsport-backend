...
class Utility
{
    ...
    {
        $options = [
            'messageTitle' => ($type == 'error') ? trans('generals.error_alert_title') : trans('generals.success_alert_title'),
            'messageIcon' => ($type == 'error') ? 'glyphicon glyphicon-remove-circle' : 'glyphicon glyphicon-ok-circle',
            'message' => ($type == 'error') ? trans('generals.error_default') : trans('generals.success_default'),
    ...
    public static function createImage($image, $name, $extension, $path)
    {
        ...
        if ($im !== false) {
            switch($extension)
            {
                case 'png':
                    ...
                    imagepng($im,  $_SERVER['DOCUMENT_ROOT'] . '/' . $path . '/' . $name . '.png');
                    return true;
                case 'jpeg':
                    header('Content-Type: image/jpeg');
                    imagejpeg($im,  $_SERVER['DOCUMENT_ROOT'] . '/' . $path . '/' . $name . '.jpeg');
                    return true;
                default:
                    return false;
            }
